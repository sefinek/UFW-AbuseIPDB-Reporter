...
sudo -u "$USER" pm2 start
sudo -u "$USER" pm2 startup

# Execute the command generated by pm2 startup
echo "🔧 Complete pm2 startup configuration (for $USER):"
sudo env PATH="$PATH":/usr/bin /usr/lib/node_modules/pm2/bin/pm2 startup systemd -u "$USER" --hp /opt/"$USER"
